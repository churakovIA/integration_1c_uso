...
public class RowMapperUtil {
    ...
    static <T extends AbstractDocument> T fill(ResultSet rs, T doc) throws SQLException {
        doc.setRef(UuidUtils.asUuid(rs.getBytes("_IDRRef")));
        doc.setNumber(rs.getString("_Number"));
        doc.setDateTime(rs.getTimestamp("_Date_Time").toLocalDateTime().minus(2000, ChronoUnit.YEARS));
    ...
    static RowMapper<CoordinationStatus> getCoordinationStatusesRowMapper(){
        return (rs, rowNum) -> {
            ...
            status.setPeriod(rs.getTimestamp("_Period").toLocalDateTime().minus(2000, ChronoUnit.YEARS));
